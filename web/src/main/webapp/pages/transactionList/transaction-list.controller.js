(function ($) {
    ...
    pinpointApp.constant("TransactionListConfig", {
        ...
        transactionIndexListUrl: "transactionIndexList.pinpoint",
        recallUrl: "recall.pinpoint",
        MIN_TRANSACTION_LIST_HEIGHT: 75,
        MAX_FETCH_BLOCK_SIZE: 100,
        TRANSACTION_LIST_HANDLE_POSITION: "transactionList.resizer"
    });

    pinpointApp.controller("TransactionListCtrl", ["TransactionListConfig", "$scope", "$location", "locationService", "$routeParams", "$rootScope", "$timeout", "$window", "$http", "webStorage", "TimeSliderVoService", "TransactionDaoService", "AnalyticsService", "helpContentService", "CommonAjaxService", "PreferenceService",
        function (cfg, $scope, $location, locationService, $routeParams, $rootScope, $timeout, $window, $http, webStorage, TimeSliderVoService, oTransactionDaoService, analyticsService, helpContentService, CommonAjaxService, PreferenceService) {
            ...
            function fetchNext() {
                getTransactionList(getQuery(), loadData);
            }
            ...
            $scope.setPeriod = function setPeriod(period) {
                $scope.period = period;
            }
            ...
            $scope.searchTrace = function () {
                var query = "";
                query += "userId=" + ($scope.userId == null ? "" : $scope.userId);
                query += "&traceId=" + ($scope.traceId == null ? "" : $scope.traceId);
                query += "&rpc=" + ($scope.rpc == null ? "" : $scope.rpc);
                query += "&applicationId=" + ($scope.applicationSelected == null ? "" : $scope.applicationSelected);
                query += "&period=" + ($scope.period == null ? "" : $scope.period);
                $scope.sidebarLoading = true;
                $http.post(cfg.transactionIndexListUrl, query, {
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                }).success(function (data, status) {
                    $scope.$emit('transactionTableDirective.clear');
                    oTimeSliderVoService = new TimeSliderVoService();
                    $scope.sidebarLoading = false;
                    loadData(data)
                }).error(function (data, status) {
            ...
            $scope.$on('transactionTableDirective.processRecall', function (event, isProcess) {
                $scope.sidebarLoading = isProcess;
            });
