(function () {
     ...
     * @class
     */
    pinpointApp.directive("transactionTableDirective", ["$window", "$http", "helpContentTemplate", "helpContentService", "AnalyticsService", "CommonUtilService", function ($window, $http, helpContentTemplate, helpContentService, AnalyticsService, CommonUtilService) {
        return {
            restrict: 'EA',
            replace: true,
            ...
            link: function postLink(scope, element, attrs) {
                ...
                appendTransactionList = function (transactionList) {
                    if (scope.transactionList.length > 0) {
                        ...
                        scope.transactionOrderBy = 'number';
                        scope.transactionReverse = false;
                    }
                 ...
                 * @param transaction
                 */
                scope.recall = function (transaction) {
                    if (transaction.traceId == null || transaction.applicationId == null) {
                        return;
                    }
                    var query = "traceId=" + transaction.traceId;
                    query += "&applicationId=" + transaction.applicationId;

                    scope.sidebarLoading = true;
                    $http.post("/recall.pinpoint", query, {
                        ...
                        scope.sidebarLoading = false;
                        $window.alert("Failed to fetching the request information:" + data);
                    });
                };
